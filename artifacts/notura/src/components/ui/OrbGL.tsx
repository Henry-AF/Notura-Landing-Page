...
export function OrbGL({
  ...
  useEffect(() => {
    ...
    function resize() {
      ...
      (program.uniforms.iResolution as any).value.set(gl.canvas.width, gl.canvas.height, gl.canvas.width / gl.canvas.height);
    }
    ...
    const update = (t: number) => {
      ...
      lastTime = t;
      (program.uniforms.iTime as any).value = t * 0.001;
      (program.uniforms.hue as any).value = hue;
      (program.uniforms.hoverIntensity as any).value = hoverIntensity;
      (program.uniforms.backgroundColor as any).value = hexToVec3(backgroundColor);
      const effectiveHover = forceHoverState ? 1 : targetHover;
      (program.uniforms.hover as any).value += (effectiveHover - (program.uniforms.hover as any).value) * 0.1;
      if (rotateOnHover && effectiveHover > 0.5) currentRot += dt * 0.3;
      (program.uniforms.rot as any).value = currentRot;
